...
let
  ...
  mkLefthookVersion =
    ...
    in
    prev.lefthook.overrideAttrs (oldAttrs: {
      inherit src;
  ...
  versions = lib.mapAttrs (_: mkLefthookVersion) manifests;
  ...
  releaseAPI = "https://api.github.com/repos/evilmartians/lefthook/releases/latest";
in
{
  lefthook = (mkLefthookVersion manifests.${latestVersion}).overrideAttrs (oldAttrs: {
    passthru = (oldAttrs.passthru or { }) // {
      inherit versions;
      latest = versions.${latestVersion};
      updateScript = prev.writeShellScript "lefthook-update-script" ''
        set -euo pipefail

        PATH=${path}

        manifestsDir="pkgs/overlays/lefthook/manifests"
        ...
        prefetchResult=$(nix flake prefetch "github:evilmartians/lefthook/v''${newVersion}" --json)
