...
let
  ...
  mkTreefmtVersion =
    ...
    in
    prev.treefmt.overrideAttrs (oldAttrs: {
      inherit src;
  ...
  versions = lib.mapAttrs (_: mkTreefmtVersion) manifests;
  ...
  releaseAPI = "https://api.github.com/repos/numtide/treefmt/releases/latest";
in
{
  treefmt = (mkTreefmtVersion manifests.${latestVersion}).overrideAttrs (oldAttrs: {
    passthru = (oldAttrs.passthru or { }) // {
      inherit versions;
      latest = versions.${latestVersion};
      updateScript = prev.writeShellScript "treefmt-update-script" ''
        set -euo pipefail

        PATH=${path}

        manifestsDir="pkgs/overlays/treefmt/manifests"
        ...
        prefetchResult=$(nix flake prefetch "github:numtide/treefmt/v''${newVersion}" --json)
